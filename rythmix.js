...
async function getSongs(){
    ...
    const selectSongs = Array.from(all_a)
    .filter( elem => elem.href.endsWith('.mp3'))

    const songs = selectSongs.map(element => element.href)
    
    const songName = selectSongs.map(element => {
      let i = element.title
      .replaceAll("(MP3_320K).mp3", "")
      .replaceAll("(MP3_160K).mp3", "")
      .replaceAll(".mp3", "")
      .replaceAll("_", "")
      .replaceAll("Video", '')
      if (i.length > 64) {
    ...
    songsButton.forEach((btn, index)=>{
        btn.addEventListener("click",function() {
            ...
            let currentSongName = songName[index];
            document.querySelector('.current-song').innerHTML = currentSongName
